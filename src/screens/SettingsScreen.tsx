      ...
      <View style={{ width: '32%', backgroundColor: colors.surface, paddingVertical: 16 }}>
        {menuItems.map(item => (
          <TouchableOpacity
            ...
            style={{ padding: 16, borderRadius: 8, marginBottom: 8, backgroundColor: selected === item.key ? colors.primary + '22' : 'transparent' }}
          >
            <Text style={{ fontWeight: 'bold', color: selected === item.key ? colors.primary : colors.text }}>{item.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      {/* Animated Content */}
