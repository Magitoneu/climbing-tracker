...
import { getAllGradeSystems, getGradeSystem } from '../services/gradeSystemService';
import { loadAndRegisterAllCustomSystems } from '../services/customGradeSystemService';
...
export default function LogScreen() {
  ...
  <View style={[{ flex: 1, backgroundColor: '#F1F5F9' }]}> 
      ...
      >
        <SessionHeaderCard date={date} onPressDate={() => { /* native date open handled by DatePickerField now */ }}>
        ...
        <TextInput
          style={{ backgroundColor: '#FFFFFF', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 16, borderWidth: 1, borderColor: '#E2E8F0', marginBottom: 16 }}
          value={duration}
  ...
  <TouchableOpacity onPress={openAddBoulder} style={{ backgroundColor: '#2563EB', paddingVertical: 14, borderRadius: 14, alignItems: 'center', marginBottom: 12 }}>
        ...
        {boulders.length === 0 && <Text style={{ color: '#64748B', marginBottom: 8 }}>None yet. Tap "Add Boulder".</Text>}
        {boulders.map((b, i) => (
        ...
        <TextInput
          style={{ backgroundColor: '#FFFFFF', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, borderWidth: 1, borderColor: '#E2E8F0', height: 80, textAlignVertical: 'top' }}
          value={notes}
        ...
        />
        <TouchableOpacity
          style={{ backgroundColor: '#0F172A', paddingVertical: 16, borderRadius: 16, alignItems: 'center', marginTop: 28, marginBottom: 12, opacity: saving ? 0.7 : 1 }}
          disabled={saving}
          onPress={async () => {
            if (saving) return;
            setLastError(null);
            ...
            try { await store.setItem('lastLoggingGradeSystem', gradeSystem as string); } catch {}
            setSaving(true);
            try {
        ...
        >
          <Text style={{ color: '#FFFFFF', fontWeight: '700', fontSize: 16 }}>{saving ? 'Saving...' : 'Save Session'}</Text>
        </TouchableOpacity>
        {lastError && (
