...
import { addCustomGradeSystem, getCustomGradeSystems, saveCustomGradeSystems, deleteCustomGradeSystem } from '../storage/customGradeSystemStore';
...
export async function upsertCustomSystem(custom: CustomGradeSystem): Promise<string> {
  ...
  if (exists >= 0) systems[exists] = payload; else systems.push(payload);
