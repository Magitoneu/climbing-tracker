...
import { migrateLocalSessions } from '../services/sessionService';
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (local.length === 0) { if (!cancelled) setMigrated(true); return; }
        const count = await migrateLocalSessions(local);
        if (!cancelled) { setAdded(count); setMigrated(true); }
      } catch (e: any) {
