...
export const CustomGradeSystemEditor: React.FC<CustomGradeSystemEditorProps> = ({ system, onSave, onCancel }) => {
  ...
  const [grades, setGrades] = useState<{ name: string; color: string }[]>(system?.grades || [{ name: '', color: '#2563eb' }]);
  ...
  return (
    <View style={{ padding: 16 }}>
      <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 12 }}>{system ? 'Edit' : 'Create'} Custom Grade System</Text>
      ...
      <TextInput
        ...
        style={{ borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, marginBottom: 12 }}
      />
      ...
      {grades.map((g, i) => (
        <View key={i} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          <TextInput
            ...
            style={{ flex: 1, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, marginRight: 8 }}
          />
          <TouchableOpacity
            onPress={() => setPickerForIndex(i)}
            style={{ width: 44, height: 36, borderRadius: 8, borderWidth: 1, borderColor: '#e5e7eb', marginRight: 8, backgroundColor: g.color || '#2563eb' }}
            accessibilityLabel={`Pick color for ${g.name || `grade ${i+1}`}`}
          />
          <View style={{ flexDirection: 'row' }}>
            <TouchableOpacity onPress={() => move(i, -1)} style={{ padding: 6 }}><Text>↑</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => move(i, +1)} style={{ padding: 6 }}><Text>↓</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => removeRow(i)} style={{ padding: 6 }}><Text style={{ color: '#ef4444' }}>✕</Text></TouchableOpacity>
          </View>
        </View>
      ))}
      ...
      {/* Color Picker Modal */}
      <Modal visible={pickerForIndex !== null} transparent animationType="fade" onRequestClose={() => setPickerForIndex(null)}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
              {PALETTE.map((c) => (
                <TouchableOpacity
                  ...
                  style={{ width: 36, height: 36, borderRadius: 6, margin: 6, backgroundColor: c, borderWidth: 1, borderColor: '#e5e7eb' }}
